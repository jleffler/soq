...
int main() {
    ...
    if ((context.full = sem_open("/full.sem", O_CREAT, 0700, 0)) == SEM_FAILED)
        err_error("sem_open(full.sem') failed\n");
    if ((context.full = sem_open("/empty.sem", O_CREAT, 0700, BUFFER_SIZE)) == SEM_FAILED)
        err_error("sem_open(empty.sem') failed\n");
