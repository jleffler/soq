...
static void sort(unsigned int *mInt, double *r, int n)
{
    unsigned int ph0;
    double ph1, ph2;
...
// Create morton key
static inline unsigned int mortoncode(unsigned int x, unsigned int y)
{
    int answer = 0;
    for (unsigned int i = 0; i < (sizeof(unsigned int) * 8) / 2; ++i)
    {
        answer |= ((x & ((unsigned int)1 << i)) << (2 * i + 0)) |
                  ((y & ((unsigned int)1 << i)) << (2 * i + 1));
    }
    return answer;
}

// Find max / min values
static double maxValue(double *r, int n, int d)
{
    double max = r[d];
    for (int k = 0; k < n; k++)
    {
...
static double minValue(double *r, int n, int d)
{
    double min = r[d];
    for (int k = 0; k < n; k++)
    {
