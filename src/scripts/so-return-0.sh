...
you are allowed to omit the `return 0;` at the end of `main()` — and only
`main()` — and zero will be returned.
...
The 'feature' was introduced largely for compatibilty with C++98, which
also allows it.  I regard that rule as an abomination, but it is part of
the C standard.  Personally, I aim to always write the return 0; or
equivalent at the end of main() — it's only an accident when I omit
it, to be fixed ASAP.
