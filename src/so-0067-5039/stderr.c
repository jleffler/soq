...
@(#)Author:         J Leffler
@(#)Copyright:      (C) JLSS 1988-2020
@(#)Derivation:     stderr.c 10.23 2020/02/25 03:34:13
*/
...
const char *(err_rcs_string)(const char *s2, char *buffer, size_t buflen)
{
    ...
    ** "$Revision: 10.23 $ ($Date: 2020/02/25 03:34:13 $)"
...
static void (err_vxf_print)(FILE *fp, int flags, int errnum, int estat,
                            const char *format, va_list args)
{
...
static void (err_vxn_print)(int flags, int errnum, int estat, const char *format, va_list args)
{
    if (errout == 0)
        errout = stderr;
