...
@(#)Version:        $Revision$
@(#)Last changed:   $Date$
@(#)Purpose:        Modified Binary Search - looking for maximum index with value less than or equal to search value
@(#)Author:         J Leffler
...
/*
** In contrast to the other searches which deal with dense search ranges
** (multiple entries with the same key value), this deals with sparse
** ranges where the value being sought may not be present at all, but
** the desire is to return the maximum index with value less than or
** equal to the search value, and for symmetry, the minimum index with
** value greater than or equal to the search value.
**
** So, we are searching for (the best index for) T in X[1..N], setting
** P to the index (or 0 if the smallest value in the array is bigger
** than T, or N+1 if the largest value in the array is smaller than T).
**
** This requires a different loop invariant.  Assume that X[0] is
** smaller than T and X[N+1] is larger than T, but those array entries
** will not be accessed.
**
** assert(forall(i = 1..N-1: X[i] < X[i+1]))
**
** Invariant 1: X[L] <= T && X[U] >  T
** Invariant 2: X[L] <  T && X[U] >= T
**
** Hence:
**
**      L := 0; U := N+1
