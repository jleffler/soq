/* SO 4103-2621 */
/* Variant 2 - factored into functions */
#include <assert.h>
#include <stdio.h>
#include <stdbool.h>
#include <string.h>
...
static bool duplicate_digits_2(int value, size_t buflen, char buffer[buflen])
{
...
bool generate_next_UDN(size_t buflen, char buffer[buflen])
{
    if (next_UDN >= MAX_ITERATION)
        next_UDN = 0;
    while (duplicate_digits_2(next_UDN++, buflen, buffer))
    {
        if (next_UDN >= MAX_ITERATION)
        {
            next_UDN = 0;
