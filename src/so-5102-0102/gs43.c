...
#if 0
/* Using `qsort()`, it is easy: */

static void gSortAll(int n, int m, int (*cmp)(const void *, const void *), ...)
{
    va_list args;
    va_start(args, cmp);
    void *array;
    while ((array = va_arg(args, void *)) != NULL)
        qsort(array, n, m, cmp);
    va_end(args);
}

/*
** You can substitute your own sort for `qsort()` — it won't be much
** different, though you'd lose the `const` qualifiers to the comparison
** function arguments.  Note that your comparator should not be
** modifying the data it compares either, and it is good to tell the
** compiler so it enforces the rule.
*/
#endif
