...
static int SubMatrixHere(size_t r, size_t c,
                         size_t rows1, size_t cols1, const int m1[rows1][cols1],
                         size_t rows2, size_t cols2, const int m2[rows2][cols2])
{
    assert(rows1 > 0 && cols1 > 0);
    assert(rows2 > 0 && cols2 > 0);
...
static int CheckSubMatrix(size_t rows1, size_t cols1, const int m1[rows1][cols1],
                          size_t rows2, size_t cols2, const int m2[rows2][cols2])
{
    assert(rows1 > 0 && cols1 > 0);
    assert(rows2 > 0 && cols2 > 0);
    ...
    size_t ub_rows = rows1 - rows2 + 1;
    size_t ub_cols = cols1 - cols2 + 1;
    for (size_t i = 0; i < ub_rows; i++)
    {
        for (size_t j = 0; j < ub_cols; j++)
        {
...
static int max_field_width(size_t r, size_t c, const int matrix[r][c])
{
    assert(r > 0 && c > 0);
...
static void dump_matrix(const char *tag, size_t rows, size_t cols, const int matrix[rows][cols])
{
...
static void test_submatrix(const char *t1, size_t r1, size_t c1, const int m1[r1][c1],
                           const char *t2, size_t r2, size_t c2, const int m2[r2][c2])
{
...
int main(void)
{
    ...
    test_submatrix("m4", 3, 3, m4, "m4", 3, 3, m4);
    test_submatrix("m4", 3, 3, m4, "m1", 4, 4, m1);
