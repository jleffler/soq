...
** Now a simple demonstration of reading the lines in a file and sorting them.
** This variant reduces the space used by using strdup() to copy the lines
** read by getline().  You can find out how much space getline() allocates
** from the returned buflen - but left to its own devices the answer is often
** 256 bytes, which is quite wasteful for short lines.
*/
